...
let fn = document.querySelector(".showEmp").getAttribute("id");
get_LabEmpRecords(fn);

activeLink(".showEmp");


document.querySelector(".showEmp").addEventListener("click", function(){
	const facilityNo = this.getAttribute("id");

	activeLink(".showEmp");
	get_LabEmpRecords(facilityNo);
});

document.querySelector(".showDtk").addEventListener("click", ()=>{
	const facilityNo = document.querySelector(".txt-facilityNo").getAttribute("id").trim();
...
document.querySelector(".showBio").addEventListener("click", () => {
	activeLink(".showBio");
});

document.querySelector(".genRco").addEventListener("click", () => {
	activeLink(".genRco");
});
...
{
	let newElement = "";
	let table_data = document.querySelector(".tbl-tblRecordList");
	...
	arrLists.reduce((acc, element) => {
		if(element.name == htmlElement)
		{
			arrSearch.reduce((acc, itemSearch) =>{
				...
				{
					document.querySelector(itemSearch.value).style.transition = "height 1s ease-out";
					document.querySelector(itemSearch.value).style.height = "300px";
				}
				else
				{
					document.querySelector(itemSearch.value).style.transition = "height 1s ease-out";
					document.querySelector(itemSearch.value).style.height = "0px";
				}
			}, {});
			
			while (table_data.firstChild) {
			    table_data.removeChild(myNode.firstChild);
			}
			// table_data.empty();
			...
			style.cssText = "color: yellow; pointer-events: none; cursor: default";

		}
		else
		{
			document.querySelector(element.name).
			style.cssText = "color: #ececec; pointer-events: all; cursor: pointer";

			// document.querySelector(element.name).css({
			// 	'color': '#ececec',
			// 	'pointer-events' : 'all', 
			// 	'cursor' : 'pointer'
			// });
	...
	function setContManualId(htmlElement)
	{
		...
		arrLists.reduce((acc, element) => {
			if(element.name == htmlElement)
			{
				document.querySelector(".cont-tsManual").removeAttribute("id");
				document.querySelector(".txtarea-query-tsManual").value = "";
				document.querySelector(".txtarea-action-tsManual").value = "";
				document.querySelector(".btn-tsManual").setAttribute("id", element.newId);
	...
	function checkContManualId(htmlElement)
	{
		...
		arrLists.reduce((acc, element) => {
			...
			{
				document.querySelector(".cont-tsManual").style.height = `0px`;
				document.querySelector(".cont-tsQa").style.height = `350px`;
...
function showTsQa(htmlElement)
{
	...
	arrLists.reduce((acc, element) => {
		if(!(element.name == htmlElement))
		{
			document.querySelector(element.cont).style.height = 0 + "px";
